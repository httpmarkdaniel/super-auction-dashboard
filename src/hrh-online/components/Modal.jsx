...
// Generic drill-down overlay for HRH Online's KPI/table click-throughs —
...
// Modal.jsx) since HRH Online is a separate module tree with its own
...
export default function Modal({ open, onClose, title, subtitle, wide = false, children }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${hrh.border}` }}>
          <div className="min-w-0">
            <div className="text-[15px] font-semibold" style={{ color: hrh.ink }}>
              {title}
            </div>
            {subtitle && (
              <div className="text-[12px] mt-0.5" style={{ color: hrh.muted }}>
                {subtitle}
              </div>
            )}
          </div>
...
export function ModalRow({ label, value, subtract = false, total = false }) {
  return (
    <div
      ...
      style={total ? { borderTop: `2px solid ${hrh.ink}`, marginTop: 6, paddingTop: 10, fontSize: 15 } : undefined}
    >
      <span style={{ color: hrh.ink2 }}>{label}</span>
      <span className="font-semibold" style={{ color: subtract ? hrh.bad : hrh.ink }}>
        {subtract ? "− " : ""}
        {value}
      </span>
    </div>
