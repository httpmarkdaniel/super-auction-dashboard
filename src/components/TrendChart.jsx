...
function CustomTooltip({ active, payload, label, unit }) {
  ...
  return (
    <div className="card px-3 py-2 text-[12.5px]">
      ...
      <div className="tabular text-ink">{formatValue(payload[0].value, unit)}</div>
    </div>
  );
}
...
export default function TrendChart({ years, values, label, unit, trend }) {
  ...
  return (
    ...
    >
      <div className="font-head text-[24px] leading-none text-ink mb-2">{formatValue(latest, unit)}</div>
