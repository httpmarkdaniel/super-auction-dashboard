...
// `accent` is accepted for existing call sites but no longer drawn — the
// uniform format has no coloured top accent on cards.
export default function StatTile({ eyebrow, value, delta, invert, sub, live, icon, pill, sparkline, methodology, onClick, extraDeltas }) {
  ...
  return (
    <Wrapper
      ...
      onClick={onClick}
      className={`relative bg-surface1 border border-gridline rounded-lg shadow-card px-4 pt-3 pb-3.5 flex-1 min-w-[180px] text-left w-full ${hasTip ? "group/tip" : ""} ${onClick ? "cursor-pointer hover:border-navy/40 transition-colors" : ""}`}
    >
