...
const EMPTY_OVERVIEW = {
  heroKPIs: {
    ...
    buyersPremiumPlusFees: 0,
    buyersPremiumDeltaPct: undefined,
    lotsSold: 0,
    lotsListed: 0,
    pendingApprovalCount: 0,
  },
  unsoldLots: { count: 0, value: 0, deltaPct: undefined, avgAgeDays: 0, fresh: 0, aging: 0, stale: 0, totalInventory: 0 },
  categoryBreakdown: [],
  branchBreakdown: [],
  channelBreakdown: [],
  hourlyTrend: [],
  bidderComposition: { newBidders: 0, returningBidders: 0, newBidderTrend: [] },
  topVendors: [],
  topBidders: [],
  reservePerformance: {
...
function buildLiveOverview(live, bidCorrectionDelta) {
  ...
  return {
    heroKPIs: {
      ...
      sellThroughDeltaPct: undefined,
      buyersPremiumPlusFees: (Number(kpis.total_buyers_premium) || 0) + (Number(kpis.total_service_fee) || 0),
      buyersPremiumDeltaPct: undefined,
...
function OverviewTab({ store, overview, lotDetailRef, rangeLabel, isLive, loading, error }) {
  const story = buildStoryline(overview, store, rangeLabel);
  ...
  return (
    <div>
      {error && (
        <div className="mb-4 px-4 py-3 rounded-lg bg-critical/10 text-toneRedText text-[13px]">
          Couldn't load dashboard data: {error}
        </div>
      )}
      {loading && !error && (
        <div className="mb-4 px-4 py-3 rounded-lg bg-plane border border-gridline text-ink text-[13px]">
      ...
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row gap-4 items-stretch">
          ...
          <HealthScoreCard {...health} trend={trend} />
        </div>
        <div className="mt-4">
      ...
      <div ref={lotDetailRef}>
        <StorySection
          ...
          insight={`Drill into every individual lot from ${
      ...
      <StorySection title="Who's driving the results" insight={story.peopleInsight}>
      ...
      <StorySection title="Where the money goes" insight={story.moneyFlowInsight} last>
...
export default function App() {
  ...
  const bidCorrectionDelta = useLiveBidCorrection(live?.overview?.auctions);
  ...
  // Ticker's "just sold" feed — real sold lots already fetched for the
  // operations table, most-recent first (api/lots.js orders by
  // date_created DESC), capped so the ticker doesn't get flooded.
  const recentSold = useMemo(
    () =>
      overview.operationsDetail
        .filter((o) => o.status === "Sold")
        .slice(0, 5)
        .map((o) => ({ lotNumber: o.lotNumber, soldPrice: o.soldPrice, store: o.branch ?? store })),
    [overview, store]
  );
    ...
    <div className="flex h-screen overflow-hidden bg-plane">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <Topbar
          ...
          unsoldCount={overview.unsoldLots.count}
          unsoldValue={overview.unsoldLots.value}
          unsoldDeltaPct={overview.unsoldLots.deltaPct}
          recentSold={recentSold}
          topVendor={overview.topVendors[0]}
          aboveReserve={overview.reservePerformance.aboveReserve}
          belowReserve={overview.reservePerformance.belowReserve}
        ...
        <div className="flex items-center gap-3 px-4 md:px-10 pt-5">
          <h1 className="flex items-center gap-2.5 text-[13px] uppercase tracking-[0.14em] font-bold text-navy shrink-0">
