...
// Shared client-side pagination control for Retail's DataTable — real
...
export default function Pagination({ page, totalPages, totalRows, pageSize, onPageChange }) {
  ...
  return (
    <div className="flex items-center justify-between flex-wrap gap-2 mt-3 pt-3" style={{ borderTop: `1px solid ${retail.border}` }}>
      <span className="text-[12px]" style={{ color: retail.muted }}>
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          style={{ background: retail.surface, color: retail.ink2, border: `1px solid ${retail.border}` }}
        >
          ‹
        </button>
        {pageNumbers(page, totalPages).map((p, i) =>
          p === "…" ? (
            <span key={`gap-${i}`} className="w-7 h-7 flex items-center justify-center text-[12px]" style={{ color: retail.muted }}>
              …
            </span>
          ) : (
            <button
              ...
              style={
                p === page
                  ...
                  : { background: retail.surface, color: retail.ink2, border: `1px solid ${retail.border}` }
              }
            >
              {p}
            </button>
