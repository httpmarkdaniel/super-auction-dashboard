    ...
    <div className="flex items-stretch">
      {yAxisLabel && (
        <div className="shrink-0 flex items-center justify-center overflow-visible" style={{ width: 18 }}>
          <span className="whitespace-nowrap text-[11px]" style={{ color: retail.ink2, transform: "rotate(-90deg)" }}>
            {yAxisLabel}
          </span>
        </div>
      )}
      <div className="flex-1 min-w-0">
        {children}
        {xAxisLabel && (
          <div className="text-center text-[11px] mt-1" style={{ color: retail.ink2 }}>
            {xAxisLabel}
          </div>
        )}
      </div>
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: angled ? 12 : 0 }}>
        ...
        <XAxis
          dataKey={xKey}
          tick={{ fontSize: 11, fill: retail.ink2 }}
          axisLine={{ stroke: retail.border }}
          tickLine={false}
          angle={xAxisAngle}
          ...
          height={xAxisHeight}
        />
        <YAxis tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={false} tickLine={false} tickFormatter={valueFormatter} width={64} />
        <Tooltip content={<ChartTooltip valueFormatter={valueFormatter} />} />
        <Legend wrapperStyle={{ fontSize: 12 }} />
        {series.map((s, i) => (
          <Line key={s.key} type="monotone" dataKey={s.key} name={s.name} stroke={s.color || retail.series[i % retail.series.length]} strokeWidth={2} dot={false} />
        ))}
      </LineChart>
    </ResponsiveContainer>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      ...
      >
        ...
        <Bar yAxisId="orders" dataKey="orders" name="Orders" fill={retail.series[0]} radius={[2, 2, 0, 0]} maxBarSize={20} />
        <Bar yAxisId="orders" dataKey="units" name="Units Sold" fill={retail.accent} radius={[2, 2, 0, 0]} maxBarSize={20} />
        <Line yAxisId="gmv" type="monotone" dataKey="gmv" name="GMV" stroke={retail.good} strokeWidth={2.5} dot={false} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        <CartesianGrid stroke={retail.border} vertical={false} />
        <XAxis dataKey="dateLabel" tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={{ stroke: retail.border }} tickLine={false} />
        <YAxis yAxisId="count" tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={false} tickLine={false} allowDecimals={false} width={36} />
        <YAxis
          yAxisId="rate"
          orientation="right"
          tick={{ fontSize: 11, fill: retail.ink2 }}
        ...
        <Legend wrapperStyle={{ fontSize: 12 }} />
        <Bar yAxisId="count" dataKey="fulfilled" name="Fulfilled" stackId="orders" fill={retail.good} radius={[0, 0, 0, 0]} maxBarSize={24} />
        <Bar yAxisId="count" dataKey="cancelled" name="Cancelled" stackId="orders" fill={retail.bad} radius={[0, 0, 0, 0]} maxBarSize={24} />
        <Bar yAxisId="count" dataKey="awaiting" name="Awaiting" stackId="orders" fill={retail.muted} radius={[2, 2, 0, 0]} maxBarSize={24} />
        <Line yAxisId="rate" type="monotone" dataKey="completionRate" name="Completion Rate" stroke={retail.accent} strokeWidth={2.5} dot={false} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        ...
        <Line yAxisId="rate" type="monotone" dataKey={rateKey} name={rateName} stroke={retail.accent} strokeWidth={2.5} dot={false} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
...
function PairedComboTooltip({ active, payload, label, barValueFormatter, lineValueFormatter, lineLabel }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="rounded-md px-3 py-2 text-[12px] max-w-[300px]" style={{ background: retail.navy, border: `1px solid ${retail.navyBorder}`, color: "#fff" }}>
      <div className="font-semibold mb-1">{label}</div>
      {payload.map((p) => {
...
export function PairedBarLineChart({
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        <CartesianGrid stroke={retail.border} vertical={false} />
        <XAxis dataKey={xKey} tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={{ stroke: retail.border }} tickLine={false} />
        <YAxis yAxisId="bar" tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={false} tickLine={false} tickFormatter={barValueFormatter} width={60} />
        {showLines && (
          <YAxis
            yAxisId="line"
            orientation="right"
        ...
        <AreaChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          ...
          {categories.map((c, i) => (
            <Area
              ...
              stroke={c.color || retail.series[i % retail.series.length]}
              fill={c.color || retail.series[i % retail.series.length]}
              fillOpacity={0.3}
            />
          ))}
        </AreaChart>
  ...
  return (
    <div className="grid gap-4 items-center" style={{ gridTemplateColumns: `${size}px 1fr` }}>
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        ...
        {(centerValue || centerLabel) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none px-2">
            {centerValue && (
              <div className="text-[14px] font-bold leading-tight" style={{ color: retail.ink }}>
                {centerValue}
              </div>
            )}
            {centerLabel && (
              <div className="text-[10px] leading-tight" style={{ color: retail.muted }}>
                {centerLabel}
              </div>
            )}
          </div>
      ...
      <div className="flex flex-col gap-2">
        {segments.map((s) => (
          <div key={s.label} className="flex items-center gap-1.5 text-[11.5px]" style={{ color: retail.ink2 }}>
            <span className="w-2 h-2 rounded-full shrink-0" style={{ background: s.color }} />
            <span className="truncate">{s.label}</span>
            <span className="ml-auto font-semibold shrink-0" style={{ color: retail.ink }}>
...
function BubbleTooltip({ active, payload, xLabel, yLabel, xValueFormatter, yValueFormatter }) {
  ...
  if (!p) return null;
  return (
    <div className="rounded-md px-3 py-2 text-[12px] min-w-[160px]" style={{ background: retail.navy, border: `1px solid ${retail.navyBorder}`, color: "#fff" }}>
      <div className="font-semibold mb-1 flex items-center gap-1.5">
        <span className="w-2 h-2 rounded-full shrink-0" style={{ background: p.color || retail.accent }} />
        {p.label}
      </div>
    ...
    <ChartWithAxisTitles xAxisLabel={xLabel} yAxisLabel={yLabel}>
      <ResponsiveContainer width="100%" height={height}>
        <ScatterChart margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
          <CartesianGrid stroke={retail.border} />
          <XAxis
            type="number"
            dataKey="x"
            tick={{ fontSize: 11, fill: retail.ink2 }}
          ...
          <Scatter data={data} fill={retail.accent}>
            {data.map((d, i) => (
              <Cell key={i} fill={d.color || retail.accent} fillOpacity={0.75} />
            ))}
          </Scatter>
        </ScatterChart>
      </ResponsiveContainer>
      ...
      <ChartWithAxisTitles xAxisLabel={xAxisLabel}>
        <ResponsiveContainer width="100%" height={height}>
          <BarChart data={data} layout="vertical" margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid stroke={retail.border} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: retail.ink2 }} axisLine={false} tickLine={false} tickFormatter={valueFormatter} />
            <YAxis
              type="category"
              dataKey={xKey}
              tick={{ fontSize: 11, fill: retail.ink2 }}
            ...
            <Legend wrapperStyle={{ fontSize: 12 }} />
            {series.map((s, i) => (
              <Bar key={s.key} dataKey={s.key} name={s.name} fill={s.color || retail.series[i % retail.series.length]} radius={[0, 2, 2, 0]} maxBarSize={22} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </ChartWithAxisTitles>
        ...
        <BarChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          ...
          {series.map((s, i) => (
            <Bar
              ...
              name={s.name}
              stackId={stacked ? "1" : undefined}
              fill={s.color || retail.series[i % retail.series.length]}
