...
import { NEEDS_ATTENTION_DATA } from "../mockAnalytics";
...
// Exception-management view over operational issues across stores,
// products, channels, customers, and inventory — see NEEDS_ATTENTION_DATA
// in ../mockAnalytics for the (currently mock) rows this filters/sorts.
export default function NeedsAttentionCenter() {
  const [priority, setPriority] = useState("all");
  const [area, setArea] = useState("all");
  const [status, setStatus] = useState("all");
  const [sortBy, setSortBy] = useState("priority");

  const priorities = useMemo(() => [...new Set(NEEDS_ATTENTION_DATA.map((r) => r.priority))], []);
  const areas = useMemo(() => [...new Set(NEEDS_ATTENTION_DATA.map((r) => r.area))], []);
  const statuses = useMemo(() => [...new Set(NEEDS_ATTENTION_DATA.map((r) => r.status))], []);

  const rows = useMemo(() => {
    const filtered = NEEDS_ATTENTION_DATA.filter(
        ...
        {SUMMARY_CARDS.map((c) => (
          <div key={c.key} className="rounded-2xl p-3.5" style={{ background: retail.bg, border: `1px solid ${retail.border}` }}>
            ...
            <div className="text-[20px] font-extrabold mt-1" style={{ color: retail.ink }}>
              {NEEDS_ATTENTION_DATA.filter(c.match).length}
            </div>
          </div>
        ))}
      </div>
